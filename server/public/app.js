...
function shell(){
  globalThis.MemoryFramePresentation?.dispose();
  ...
  if(globalThis.MemoryCall&&!$('#openFamilyCall')){const button=document.createElement('button');button.id='openFamilyCall';button.textContent='和家人通话';button.onclick=()=>MemoryCall.open();$('#settings').before(button)}
  if(frame){mountFrameMenu();globalThis.MemoryFramePresentation?.mount()}
}
...
async function startRecording(){
  ...
  if(globalThis.MemoryAI?.busy?.())return toast('请先关闭 AI 对话再给家人录音');
...
async function poll(){
  ...
  try{
    ...
    state=reconcileHistory(nextState);globalThis.MemoryAI?.refreshProactive?.();globalThis.MemoryContact?.refresh();globalThis.MemoryCall?.refresh();stateReceivedAt=performance.now();stateRoundTripMs=Math.max(0,stateReceivedAt-pollStartedAt);syncFrameSelection();refreshManagedAudio();
    const latest=photos().at(-1)?._id;
    if(latest&&latest!==previousLatest&&(frame||!followFrame)&&!(frame&&globalThis.MemoryFramePresentation?.busy())&&!recording&&!draft.audio&&!(frameAudio&&!frameAudio.paused)&&![...document.querySelectorAll('audio')].some(a=>!a.paused))selected=latest;
    ...
    renderPhoto();renderList();refreshProcessingButton();refreshSpatialPanels();refreshExperience();reportFramePresence();globalThis.MemoryPresence?.receive(state.presenceEvent);globalThis.MemoryFramePresentation?.refresh();
...
function mountFrameMenu(){
 ...
 for(const id of ['openAI','openContact','openFamilyCall','settings']){const control=$('#'+id);if(control)menu.lastElementChild.append(control)}
