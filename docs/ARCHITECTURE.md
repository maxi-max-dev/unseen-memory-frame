...
- `devices/link2-windows/`：独立 Windows 采集器与 HTTPS 上报器文本源码。官方 SDK、运行二进制和设备私有凭据另行准备。
- `test/`：业务、权限、失效、去重、生命周期及可选浏览器测试。

驻足事件只携带事件与设备标识，不上传摄像头原始画面。传感器令牌权限仅限向绑定家庭和相框上报；不能作为家庭登录会话。相框通过现有 `state` 轮询接收短时事件，前端检查去重、可见性和 AI 忙碌状态。老人确认后，AI 读取这次邀请固定的照片内容并问一句，读图失败可继续普通聊天；不自动申请麦克风或开始录音。具体字段与时限以同版本 presence 模块、协议文档及测试为准。
